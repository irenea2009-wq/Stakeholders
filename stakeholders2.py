...
if len(st.session_state.plan) > 0:
    ...
    if st.sidebar.button("🔄 Revenir au plan complet / Exporter"):
        st.session_state.step = 4
        st.rerun()
    if st.sidebar.button("➕ Ajouter un autre stakeholder"):
...
# --- STEP 3: ADD ANOTHER? ---
    ...
    st.subheader("Dernier stakeholder ajouté :")
...
# --- FINAL DISPLAY ---
    ...
    with col2:
        if st.button("➕ Ajouter un autre stakeholder (revenir)"):
            st.session_state.step = 1
            st.rerun()